            ...
            checksData.forEach((check)=>{
                ...
                _data.read('checks',check,(er,originalCheckData)=>{
                    if(!er && originalCheckData){
                        ...
                        console.log("Error in reading on of the checks data",er);
                    }
                })
            });
        }else{
    ...
    _data.update('checks',newcheckData.id,newcheckData,(err)=>{
        if(!err){
            ...
            if(alertwarranted){
                ...
                console.log("Check outcome has not changed, no alerts needed");
            }
        }else{
            console.log("Error trying to save data to one of the checks");
        }
    });
}
...
workers.log =(originalCheckData,checkOutcome,state,alertwarranted,timeOfCheck)=>{
    ...
    _logs.append(logFileName,logString,(err)=>{
        if(!err){
            console.log("Logging to file successfull");
        }else{
...
workers.alertUserToStatusChange = (newcheckData)=>{
    ...
    helpers.sendSMSTOUSER(phone,msg,(err,res)=>{
        if(!err && res){
            ...
            console.log("Message wasnt sent",res);
        }
    })
        ...
        if(!err && logs){
            ...
            //compress the data to a different file
                ...
                _logs.compress(logId,newFileId,(err)=>{
                    if(!err){
                        //Truncate the log
                        _logs.trucate(logId,(err)=>{
                            if(!err){
                                console.log("Success trancating logFile");
                            }else{
                                console.log("Error truncating on of the logs")
                            }
                        })
                    }else{
                        console.log("Error compressing on of the logs",err);
                    }
                })
            })
        }else{

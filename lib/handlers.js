            ...
            if (!err && checksData) {
                ...
                handlers._tokens.verifyToken(token, checksData.userPhone, (tokenIsValid) => {
                    if (tokenIsValid) {
                                ...
                                _data.read('users', checksData.userPhone, function (err, usersdata) {
                                    if (!err && usersdata) {
                                        ...
                                        if(checkPosition > -1){
                                            ...
                                            callback(500,{"Error":'Could not find the check on the users object, so could not remove from the list'});
                                        }
                                    } else {
                                        console.log(err);
                                        callback(500, {
                        ...
                        callback(403,{"error":"Token Verification wasn't successfull"});
                    }
                });
            } else {
                callback(400, {
